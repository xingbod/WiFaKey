...
class AWGN:
    def __init__(self, top_config, rng_seed=None):
        ...
        if self.Cov_Noise:
            fin_cov_file = open(top_config.cov_1_2_file , 'rb')
...
class BSC:
    ...
    def channel_transmit_with_noise(self,s_mod, ch_noise):
        y_receive = np.multiply(s_mod, ch_noise) # confirm, xor need to be add negnative
        # y_receive = np.logical_xor(s_mod, ch_noise)
        return y_receive